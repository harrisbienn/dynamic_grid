...
class ColoredLog:
    ...
    def __init__(self, type: ELogTypes, message: str, severe: bool, showTime: bool, timeFormat: str,
                 textColor: colorama.Fore, bgColor: colorama.Back):
        self.type = type
...
def getDefaultTextColor(logType: ELogTypes, isSevere: bool):
    if (logType == ELogTypes.log):
        return colorama.Fore.WHITE if (isSevere) else ''
    elif (logType == ELogTypes.warn):
        return colorama.Fore.BLACK if (isSevere) else colorama.Fore.YELLOW
    elif (logType == ELogTypes.error):
        return colorama.Fore.BLACK if (isSevere) else colorama.Fore.RED
    elif (logType == ELogTypes.success):
        return colorama.Fore.BLACK if (isSevere) else colorama.Fore.GREEN
    elif (logType == ELogTypes.info):
        return colorama.Fore.BLACK if (isSevere) else colorama.Fore.BLUE


def getDefaultBgColor(logType: ELogTypes, isSevere: bool):
    if (logType == ELogTypes.log):
        return colorama.Back.BLACK if (isSevere) else ''
    elif (logType == ELogTypes.warn):
        return colorama.Back.YELLOW if (isSevere) else ''
    elif (logType == ELogTypes.error):
        return colorama.Back.RED if (isSevere) else ''
    elif (logType == ELogTypes.success):
        return colorama.Back.GREEN if (isSevere) else ''
    elif (logType == ELogTypes.info):
        return colorama.Back.BLUE if (isSevere) else ''
...
class Console:
    ...
    def __createConsoleRecord(self, type: ELogTypes, message: str, severe: bool, showTime: bool, textColor: str,
                              bgColor: str):
        return ColoredLog(
            type=type,
            message=message,
